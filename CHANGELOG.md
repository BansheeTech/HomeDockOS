...
- **2.1.6.164** (Latest): Time-bounded the onboarding window, hardening fresh-install setup against late unattended claiming.
  - **Hardened the first-run setup window** (reported by @EQSTLab, SK Shieldus). The onboarding page exists to create your username and password when none exist yet, that is its entire purpose, so by definition it has to be reachable before any credentials are set, exactly like the first-run setup of almost any self-hosted platform. What we patched was not a hole in that page but the fact that, on an instance nobody ever configured, it stayed reachable with no time limit, so a stranger could reach it and create the account before the real owner did. We've now put a clock on it. Onboarding is accepted only during the first 15 minutes after HomeDock OS starts, and a host restart reopens the window, so completing first-time setup now requires control of the machine. If you've already completed onboarding, or set your own username and password, none of this affects you, there is no way to run onboarding again once an account exists. A "Restart required" screen explains how to reopen the window. Walkthrough at [our documentation](https://docs.homedock.cloud/troubleshooting/restart-required/).
    > **TL;DR:** Our Onboarding page has to be open before your account exists, that's its whole job, we just missed putting a clock on it. Now it's a 15-minute window that only a host restart reopens, so a box nobody configured can't be claimed by the wrong person. Turns out "we'll set it up later" needed an expiry date. Own the machine, own the setup. Stay safe out there selfhosters!

---

- **2.1.6.162**: Graceful mDNS degradation so HomeDock OS boots on lockdown systems, plus axios, cryptography, and DOMPurify security dependency upgrades.
  - **HomeDock OS now boots on lockdown systems where mDNS isn't available.** The `homedock.local` discovery feature relies on a component that some hosts block (like Windows 11 with Smart App Control), and that used to crash the server on startup. Now it simply skips mDNS with a friendly note and boots normally — `homedock.local` was always a nice-to-have, not a requirement. Also cleaned up the startup code that handled it.
  - **Patched 4 axios advisories** by upgrading `axios` (npm) from 1.17.0 to 1.18.0 in `package.json` (and re-resolving `package-lock.json`), closing an inherited-proxy MitM, a Basic-auth prototype-pollution gadget, a `NO_PROXY` bypass, and a form-serializer DoS. None of the four carry a CVE ID; all were extracted from Dependabot.
  - **No CVE associated (extracted from Dependabot)** — Inherited proxy after interceptor config cloning: the Node.js HTTP adapter re-reads `config.proxy` through the prototype chain when `Object.prototype.proxy` is polluted and a request interceptor returns a `{...config}` / `Object.assign({}, config)` clone, which re-materialises axios' hardened null-prototype config back into a plain object and undoes the earlier merge hardening. Routes affected plaintext requests through an attacker-controlled proxy, exposing `Authorization`, axios-generated Basic auth, method, absolute URL, Host and request body (CWE-200 / CWE-1321).
  - **No CVE associated (extracted from Dependabot)** — Basic-auth injection via prototype pollution: an `auth: {}` (or partial `auth`) object with polluted `Object.prototype.username` / `.password` is read subfield-by-subfield down the prototype chain and used to build the outbound `Authorization: Basic …` header, letting an attacker inject or override the credentials axios puts on the request (CWE-1321).
  - **No CVE associated (extracted from Dependabot)** — `NO_PROXY` bypass for `0.0.0.0`: `isLoopback('0.0.0.0')` returns false, so `NO_PROXY=localhost,127.0.0.1,::1` fails to cover `http://0.0.0.0:<port>/` and the request is routed through the configured proxy instead of bypassing it, an SSRF / local-service-disclosure primitive when the request URL or a followed redirect target is attacker-influenced (CWE-183 / CWE-918).
  - **No CVE associated (extracted from Dependabot)** — Form-serializer `maxDepth` bypass via the `{}` metatoken: a top-level key ending in `{}` is handed straight to native `JSON.stringify()` before the `formSerializer.maxDepth` guard can inspect the nested structure, so a deeply nested value throws a raw `RangeError: Maximum call stack size exceeded` and denies the affected request path (CWE-674).
  - **Patched the cryptography OpenSSL advisory** by upgrading `cryptography` (pip) from 46.0.7 to 48.0.1 in `requirements.txt`. The pyca/cryptography wheels published to PyPI statically link OpenSSL, and every wheel before 48.0.1 bundles a copy vulnerable to the OpenSSL 2026-06-09 security advisory (out-of-bounds read, CWE-125 / CWE-1395). No CVE associated; extracted from Dependabot.
  - **Bumped DOMPurify from 3.4.11 to 3.4.12** (lockfile-only, already inside the existing `^3.4.1` range) to close `GHSA-c2j3-45gr-mqc4`, where `CUSTOM_ELEMENT_HANDLING` lets an allowed custom element skip the `afterSanitizeElements` hook. This one wasn't in the Dependabot batch — it surfaced from `npm audit` while re-resolving the lockfile; low severity, no CVE associated.
  - The rest of the June/July Dependabot batch was already satisfied by the versions resolved in the tree, so nothing to change beyond confirming it: `form-data` ≥ 4.0.6 (CVE-2026-12143, CRLF injection), `esbuild` ≥ 0.28.1 (Windows servedir path traversal), `zeroconf` ≥ 0.149.16 (CVE-2026-48045 TC-deferred queue exhaustion and CVE-2026-48487 rdlength cache corruption), and the two earlier DOMPurify `setConfig` / Trusted-Types advisories.
    > **TL;DR:** Half the work this round was proving the alerts were already dead. The other half was axios shipping four fresh prototype-pollution and proxy-routing gadgets in a single dot release, none of them dignified with a CVE. Keep your `Object.prototype` clean out there folks.

- **2.1.6.148**: axios and zeroconf security dependency upgrades, closing 11 CVEs.
  - **Patched 8 axios CVEs** by upgrading `axios` (npm) from 1.15.2 to 1.17.0 in `package.json`, closing a full sweep of prototype-pollution gadgets, proxy-credential leaks, a NO_PROXY bypass, a ReDoS, and an unenforced size-limit DoS.
  - **CVE-2026-44494** (Critical) — Full Man-in-the-Middle via a prototype-pollution gadget in `config.proxy` (CWE-1321 / CWE-441). Because `proxy` is absent from axios defaults, the merged config has no own `proxy` property, so any `Object.prototype` pollution in the dependency tree is read straight through by the HTTP adapter and routes every request through an attacker-controlled proxy, exposing all headers, credentials, and bodies.
  - **CVE-2026-44489** — Proxy-Authorization header injection via prototype pollution; an incomplete-null-prototype patch bypass of the 1.15.2 fix where nested objects from `utils.merge()` still inherit from `Object.prototype` (CWE-1321 / CWE-113).
  - **CVE-2026-44490** — DoS and header injection via read-side prototype-pollution gadgets in the axios merge functions: polluted `Object.prototype.get`/`.set` throw on every request, while polluted method buckets inject headers onto the wire (CWE-1321).
  - **CVE-2026-44488** — Allocation of resources without limits: the `fetch` adapter did not enforce configured `maxContentLength` / `maxBodyLength`, allowing oversized request/response bodies to exhaust memory (CWE-770).
  - **CVE-2026-44496** — Regular-expression Denial of Service (ReDoS) via XSRF cookie-name injection; the cookie name was interpolated into a regex without escaping metacharacters (CWE-400 / CWE-1333).
  - **CVE-2026-44486** — `Proxy-Authorization` header leaks to the redirect target when a proxied request is re-evaluated to a direct connection and the stale header is not cleared (CWE-200).
  - **CVE-2026-44487** — `Proxy-Authorization` credential leak to the origin server across an HTTP-to-HTTPS redirect in the Node.js HTTP adapter (CWE-201).
  - **CVE-2026-44492** — `shouldBypassProxy` does not normalise IPv4-mapped IPv6 addresses (`::ffff:7f00:1`), allowing a `NO_PROXY` bypass and SSRF against loopback and cloud-metadata endpoints; an incomplete fix for CVE-2025-62718 (CWE-918).
  - **Patched 3 zeroconf CVEs** by upgrading `zeroconf` (pip) from 0.148.0 to 0.149.7 in `requirements.txt`. All three are LAN-local denial-of-service vectors reachable by any unauthenticated host on the local link (UDP/5353).
  - **CVE-2026-47180** — Unbounded recursion in the DNS compression-pointer decoder; a ~3 kB packet with ~1500 chained pointers blows past CPython's recursion limit, and the resulting `RecursionError` escaped the decode error handling (CWE-674).
  - **CVE-2026-47183** — Unbounded exception-dedup state retained raw packet buffers via traceback frame locals, pinning ~9 KB per unique malformed packet until process exit and enabling memory exhaustion (CWE-400).
  - **CVE-2026-47184** — Unbounded DNS record cache with no entry cap; a multicast flood of uniquely-named records accumulates without bound and OOM-kills memory-constrained deployments (CWE-400 / CWE-770).
  - **Upgraded Vite from 7 to 8 (`^8.0.16`) and Vue to 3.5.35.** Vite 8 is a major jump, not a routine bump: it swaps the bundler from Rollup to Rolldown and pulls lightningcss into the CSS pipeline. Flagging it because the two entries below are direct consequences of that move.
  - **Bumped `@vitejs/plugin-basic-ssl` from 2.1.4 to 2.3.0** (devDependency) so its peer range covers Vite 8, clearing the `ERESOLVE overriding peer dependency` warning. Not security-related, just tidying up the tree while we were in here.
  - **Pinned `build.cssMinify` to `esbuild` to fix transparent window backgrounds.** Vite 8 defaults its CSS minifier to lightningcss, which drops the unprefixed `backdrop-filter` and keeps only the `-webkit-` variant, and current Chromium needs the unprefixed property, so every frosted-glass surface (windows, dialogs, context menus, notifications) lost its blur and read as see-through on Noir and Default. The themes were never touched; the glass effects live as raw CSS in component `<style scoped>` blocks, and lightningcss was mangling them at minify time. Pinned `cssMinify: 'esbuild'` until [lightningcss#695](https://github.com/parcel-bundler/lightningcss/issues/695) lands; reported upstream against both lightningcss and Vite.
    > **TL;DR:** What a time to be in cybersecurity, where AI writes both the exploits and the patches faster than most of us can read the CVE descriptions. Stay frosty out there folks.

- **2.1.6.144**: urllib3 security dependency upgrade.
...
- **2.1.6.142**: Full UI i18n across 10 languages, regional time and week formats, wallpaper transitions, onboarding and sweep fixes.
  ...
  - **App Store branding stays "App Store"** in Latin-script locales and gets the locally-known equivalent (应用商店, アプリストア, 앱 스토어, Магазин приложений, Магазин застосунків) in non-Latin ones, instead of literal calques nobody uses.
  - **Theme and language pickers redesigned** as consistent rounded pills with native tooltips, ellipsis fallback, and zero layout shift on hover, focus or selection. Killed the antd RadioGroup + multiple competing transitions that caused flickering, the 1px translate-on-hover that felt jittery, and the icon `v-if` reflow that made text wobble when selecting an option.
  - **Defense-in-depth locale sanitization**. Even though we control the JSON files, we don't trust ourselves either: every locale gets sanitized at load time with DOMPurify (already in the bundle for Notepad's markdown), allowing only `<strong>`, `<em>`, `<b>`, `<i>`, `<code>`, `<br>`, `<u>` and stripping every script, iframe, on\*= handler, javascript: URI, control character, malformed placeholder, and oversized value. If a community translation PR ever sneaks `<img src=x onerror=...>` into a string that lands in a `v-html`, the sanitizer catches it even before vue-i18n sees it. The locale degrades gracefully to keys-as-English fallback, the app keeps running, and you get a `console.warn` pointing at the offender. Better to find a stripped tag in dev tools than a popped XSS in production.
  - **AppExplorer is now AppFinder** across the whole codebase (component file, registration, system icon map, translations).
  - **AppFinder is now a router, not a downloader**. Clicking a file or folder in Finder no longer triggers an inline download with its own progress bar, it focuses the existing File Explorer window (or opens a new one) at the file's parent path, with the file pre-selected and scrolled into view. Cuts ~100 lines of duplicated download/progress logic, drops two `axios` paths, and keeps a single source of truth for file operations: AppFinder finds, AppFileExplorer operates.
  - **No more 1 GB ceiling on uploads, anywhere**. Storage, Drop Zone, App Drive and Disks+ now accept files of any size by splitting them into 5 MB chunks on the client and reassembling them on the server. Each chunk lands directly at its final destination as it arrives, so when the last one finishes the file just gets renamed to its real name, instant, no copy step, no second pass over the data. Network blips or closed browser tabs leave the partial upload hidden in the destination folder and a background cleanup wipes it after 60 minutes if nobody comes back to finish. Pushing a 4 GB MP4 to a USB drive in Disks+, or any file larger than the previous Hypercorn body cap to any backend, now just works.
  - **Drop Zone encrypts each chunk in memory before it touches disk**. The chunked migration kept Drop Zone's no-plaintext-on-disk guarantee intact: each arriving chunk is encrypted with AES-GCM in RAM and only the ciphertext is written out, never the original bytes. The final file is bit-identical to the legacy single-shot v3 format (nonce + ciphertext + tag), so the existing decryptor reads old and new files the same way, no migration, no version flag, no dual reader.
  - **Editors keep their own simpler save endpoint**. Brusher, Code and Notepad save through `/api/{backend}/edit`, capped at 100 MB and separate from the chunked path used by the File Explorer queue. For files this small the round-trips of init > chunk > finalize would just be overhead, so each editor still hands a single `FormData` to one URL and gets one response back.
  - **In-flight uploads stay invisible everywhere they could leak**. A centralized filter hides the temporary `.homedock-upload-*` assembly file from every listing, recursive search and multi-file zip download across all four backends, so a parallel File Explorer window pointing at the same destination directory never sees a half-finished upload, and a "Download as zip" of the parent folder never bundles one in.
  - **Top-level await in mount points**. Bumped `tsconfig` target from `es2016` (a 2016 default that was sitting around for no good reason) to `es2022`, which lets the bootstrap do `await bootstrapI18n(); app.mount(...)` directly instead of nesting callbacks. Vite's emit target is unaffected.
  - **Hardcoded English strings in Finder swept out**. System app names, descriptions, App Store categories, relative time labels (`5m ago`, `2h ago`), DropZone descriptions, and window titles all flow through `t()` now and re-render reactively when the user switches language.
  - **Cross-component navigation in File Explorer**. Added `initialPath` and `initialFileName` props (alongside the existing `initialLocation`/`initialContainer`/`initialMountIndex`/`initialDiskId`) so any caller can open the explorer at a precise location, with a specific item selected. Reuses `windowStore.openFileInApp(...)` so a second click reuses the open window instead of stacking new ones — the "one app, one place" pattern of any reactive UI worth its salt.
  - **International folder names on the desktop**. The folder-name sanitizer was a Latin-only allowlist (`a-zA-Z0-9` plus a hand-picked set of Spanish accented characters), which silently stripped every CJK, Cyrillic, Arabic, Hebrew, Greek, etc. character — meaning `새 폴더`, `新建文件夹`, `新しいフォルダ`, `Новая папка` all collapsed to empty strings and fell back to "New Folder" in English. Replaced with Unicode property classes (`\p{L}\p{N}\p{M}` with the `u` flag), so any alphabet on Earth works while filesystem-unsafe characters (`/ \ : * ? " < > |`, control codes) are still blocked. Length cap bumped from 20 to 32 to give CJK a fair shot at a useful name.
  - **Layered wallpaper transitions on Aero+**. Switching wallpapers (preset or custom) and toggling Noir > Aero+ now plays a smooth elastic fade-and-zoom over the previous wallpaper. Each change layers on top of the previous one, so rapid clicks keep the animation flowing instead of cutting between states, and the stack collapses back to a single layer once the topmost has settled.
  - **Clock format and week start, picked once and respected everywhere**. Two new options in Settings > Theme: 24-hour vs 12-hour (AM/PM), and weeks starting Monday or Sunday. The clock format flows through the taskbar tray clock, the calendar popup event list, the Calendar app event sidebar, the World Clock cards, the System Logs login timestamps, and the System Logs performance chart axes. The week start rotates the day-of-week header and the cell offset for both the Calendar app and the taskbar dropdown calendar.
  - **First-run onboarding**. Fresh installs no longer ship with the legendary `user / passwd` duo baked in. New installs now land on a guided five-step setup flow that captures language, username and password, theme and wallpaper, time format and week start, and a final review before sign-in. The flow is fully translated across all 10 locales and lives at `/onboarding`.
  - **Passwords now work in any language**. The 30-character cap was counting characters, but the underlying password engine measures bytes so a Japanese, Korean, Chinese, Hindi, Cyrillic, Arabic, Hebrew, Thai, or emoji password could quietly overflow it and either break onboarding with a generic error or fail to log in afterwards with an opaque "decryption failed". The input fields now meter bytes in real time as you type, so the limit naturally adjusts to your alphabet.
  - **Disks+ uploads >1 MB no longer get rejected**. The `/api/disksplus/upload` endpoint was missing from the `endpoint_limits` table since 2.1.4.682, so anything bigger than the 1 MB default got `413 Request Entity Too Large` at the ASGI middleware before the body finished arriving. Now registered at 1 GB alongside Storage, Drop Zone, and App Drive.
  - **Disks+ unlock session slides on every chunk**. Long uploads no longer expire mid-way: each chunk that lands in Disks+ calls authorize_request server-side, which validates the session is still alive and pushes its expiry to the full TTL. The local UI countdown matches per chunk. A 4 GB MP4 to a slow USB drive now finishes regardless of how short you set the timeout, while a paused upload past TTL still gets a clean `401 unlock_required` on the next chunk attempt.
...
- **2.0.2.282**: Fixed SSL certificate verification for self-signed certificates in application endpoint detection.
  ...
  - Thanks to **@Flippy** for reporting this issue and helping make HomeDock OS better! Our community is incredible!
    > **TL;DR:** Why this happens: Self-signed certificates can't be verified because they're user-signed, not CA-signed. We always attempt full certificate validation first (ensuring Let's Encrypt and production certificates maintain strict security), and only falling back to accept self-signed certs when verification fails (standard practice for local/dev environments). This way, legitimate certificates stay fully protected while self-signed ones remain functional.
