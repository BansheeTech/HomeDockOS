...
# Chequear nuevas imágenes uando subprocess ################ EXPERIMENTAL
# FALLA MUCHAS VECES, stop_and_update_container NO ES SECUENCIAL SI DETECTAMOS
# LA IMAGEN, HACE STOP, REMOVE Y UP -D ANTES DE FINALIZAR EL PULL
# Hay que buscar otra forma de hacerlo pero probablemente haya k modificar
# El hijode satán que es hd_dockerAPIUpdateContainer.js, y lo odio a muerte
# Y ESTOY CANSAO, así que lo dejo como estaba que tampoco está tan mal mientras tanto
# Me está empezando a quedar grande este proyecto :(
...
#         process = subprocess.Popen(["docker", "pull", docker_image], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

#         up_to_date = False  # Controlar si la imagen está actualizada

#         # Leer solo las primeras 4 líneas de la salida
#         for _ in range(4):
...
#         # Terminar proceso si sabemos que la imagen es updotdate
#         if up_to_date:
#             process.terminate()
#         else:
#             new_images_containers.append(name)
