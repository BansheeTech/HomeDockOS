  ...
  <div class="flex flex-col h-full overflow-hidden">
    <div class="px-4 md:px-4 py-5 border-b" :class="themeClasses.explorerHeaderBorder">
      <div class="relative flex items-center gap-3">
        ...
        <input ref="searchInputRef" v-model="searchQuery" type="text" :placeholder="$t('Search apps, files, and more...')" class="flex-1 py-3 pr-4 pl-12 rounded-lg border text-sm outline-none transition-all duration-200" :class="[themeClasses.explorerSearchInput, themeClasses.explorerSearchInputText, themeClasses.explorerSearchInputFocusRing]" @keydown="handleKeyDown" />
    ...
    <div class="flex gap-2 px-4 md:px-4 py-4 border-b overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" :class="themeClasses.explorerHeaderBorder">
      <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" class="flex items-center gap-2 md:gap-1.5 px-4 md:px-3 py-2 rounded-lg border-none text-sm md:text-[0.8125rem] font-medium cursor-pointer transition-all duration-150 whitespace-nowrap max-[480px]:min-w-12 max-[480px]:px-2" :class="activeTab === tab.id ? themeClasses.explorerTabActive : [themeClasses.explorerTabInactive, themeClasses.explorerTabHover]">
        ...
        <span class="max-[480px]:hidden">{{ $t(tab.label) }}</span>
    ...
    <div class="relative flex-1 overflow-y-auto">
      <div v-if="!searchQuery.trim() && activeTab === 'all'" class="flex flex-col items-center justify-center px-8 py-16 text-center">
        ...
        <p :class="themeClasses.explorerEmptyText" class="text-base font-medium mb-2 text-balance">{{ $t("Start typing to search across HomeDock OS") }}</p>
        <div class="mt-8 text-left max-w-md" :class="themeClasses.explorerQuickTips">
          <p class="text-sm font-semibold mb-2">{{ $t("Quick tips:") }}</p>
          <ul class="list-none p-0 text-[0.8125rem] leading-relaxed">
            <li class="before:content-['•'] before:mr-2 before:opacity-50">{{ $t("Search for apps, files, or available software") }}</li>
            <li class="before:content-['•'] before:mr-2 before:opacity-50">{{ $t("Use filters to narrow down results") }}</li>
            <li class="before:content-['•'] before:mr-2 before:opacity-50">{{ $t("Select a category above to browse items") }}</li>
          </ul>
        </div>
      </div>
      ...
      <div v-else-if="filteredResults.length === 0" class="flex flex-col items-center justify-center px-8 py-16 text-center">
        ...
        <template v-if="searchQuery">
          <p :class="themeClasses.explorerEmptyText" class="text-base font-medium mb-2">{{ $t('No results found for "{query}"', { query: searchQuery }) }}</p>
          <p :class="themeClasses.explorerEmptySubtext" class="text-sm">{{ $t("Try different keywords or check your spelling") }}</p>
        </template>
          ...
          <div class="flex flex-col gap-2">
            <div v-for="(item, itemIndex) in group.items" :key="item.id" @click="handleItemClick(item)" @mouseenter="selectedIndex = getGlobalIndex(groupIndex, itemIndex)" class="relative flex items-center gap-4 px-3 py-3 rounded-lg cursor-pointer transition-all duration-150 overflow-hidden" :class="[themeClasses.explorerResultItem, themeClasses.explorerResultItemHover, selectedIndex === getGlobalIndex(groupIndex, itemIndex) ? themeClasses.explorerResultItemSelected : '']">
              ...
              <div class="flex-shrink-0 px-2 py-1 rounded text-[0.625rem] font-semibold uppercase z-[1]" :class="getBadgeClass(item.type)">
                {{ $t(getTypeBadge(item.type)) }}
              </div>

              <div class="flex-shrink-0 z-[1]">
                <button @click.stop="handleItemClick(item)" class="p-2 rounded-md border-none cursor-pointer transition-all duration-150" :class="[themeClasses.explorerActionButton, themeClasses.explorerActionButtonHover]" :title="$t(getActionLabel(item.type))">
    ...
    <StatusBar :icon="fileSearchIcon" :message="$t('Finder')" :info="`${totalItems} ${totalItems === 1 ? $t('item') : $t('items')} ${$t('available')}`" :showHelp="true">
      <template #help>
...
const allResults = computed<SearchResult[]>(() => {
  ...
  utilitiesApps.value.forEach((util) => {
    results.push({
      ...
      type: "utility",
      name: t(util.name),
      description: t(util.description),
      icon: util.icon,
      category: "Utilities",
      score: 0,
  ...
  storageFiles.value.forEach((file) => {
    ...
    const parentDir = file.fullPath.split("/").slice(0, -1).join("/");
    results.push({
      ...
      score: 0,
      action: () => openInExplorer("storage", parentDir, file.fullPath),
    });
  });
  ...
  dropzoneFiles.value.forEach((file) => {
    ...
    results.push({
      ...
      name: file.name,
      description: t(isFolder ? "Folder" : "Encrypted file"),
      icon: isFolder ? folderIcon : getFileIcon(file.name),
      category: "Drop Zone",
      size: file.size,
      modified: file.modified,
      score: 0,
      action: () => openInExplorer("dropzone", "", file.name),
    });
  });
...
function getRelativeTime(modified: string | number): string {
  ...
  if (diffInSeconds < 3600) return t("{n}m ago", { n: Math.floor(diffInSeconds / 60) });
  if (diffInSeconds < 86400) return t("{n}h ago", { n: Math.floor(diffInSeconds / 3600) });
  if (diffInSeconds < 2592000) return t("{n}d ago", { n: Math.floor(diffInSeconds / 86400) });
  return t("{n}mo ago", { n: Math.floor(diffInSeconds / 2592000) });
}
...
function getActionIcon(type: string): any {
  const icons: Record<string, any> = {
    ...
    docker: playIcon,
    "storage-file": folderOpenOutlineIcon,
    file: folderOpenOutlineIcon,
...
function getActionLabel(type: string): string {
  const labels: Record<string, string> = {
    ...
    utility: "Open",
    docker: "Open",
    "storage-file": "Show in File Explorer",
...
function openInExplorer(location: "storage" | "dropzone", path: string, fileName?: string) {
  windowStore.openFileInApp("fileexplorer", {
    data: { initialLocation: location, initialPath: path, initialFileName: fileName },
  });
}
...
function openAppStore(appName: string) {
  ...
  if (!app) {
    message.error(t("App {name} not found", { name: appName }));
    return;
  }
...
function getEmptyMessage(): string {
  const messages: Record<string, string> = {
    all: t("No items available"),
    apps: t("No apps found"),
    files: t("No files in Drop Zone"),
    available: t("All apps are already installed"),
  };
  return messages[activeTab.value] || t("No items found");
}

function getEmptySubtext(): string {
  const subtexts: Record<string, string> = {
    all: t("Start by installing apps or uploading files"),
    apps: t("Install apps from the App Store to get started"),
    files: t("Upload files to Drop Zone to see them here"),
    available: t("Great! All available apps are installed"),
  };
