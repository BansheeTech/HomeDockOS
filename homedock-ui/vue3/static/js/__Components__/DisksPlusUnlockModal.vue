...
<template>
  <AppDialog :visible="visible" :title="'Unlock Disks+'" :ok-text="loading ? 'Verifying' : 'Unlock'" :cancel-text="'Cancel'" :ok-cancel="true" :close-on-ok="false" :loading="loading" :ok-disabled="!password" :icon="lockIcon" type="confirm" @ok="handleSubmit" @cancel="handleCancel" @update:visible="onVisibleUpdate">
    <div class="flex flex-col gap-3">
      <p class="text-sm leading-relaxed m-0" :class="themeClasses.notTextDown">{{ $t("Disks+ gives you access to") }} <strong>{{ $t("every mounted disk") }}</strong> {{ $t("on this machine, including system paths protected behind danger zones. Enter your password to start {session}.", { session: sessionMinutes > 0 ? $t("a {n}-minute session", { n: sessionMinutes }) : $t("a session with no timeout") }) }}</p>

      <div v-if="dockerLimited" class="text-xs rounded-md px-3 py-2" :class="[themeClasses.warningBg, themeClasses.warningText]">
        <strong>{{ $t("HomeDock OS is running in Docker:") }}</strong> {{ $t("Sadly, only paths bind-mounted into the container are visible. Add the host paths you want to browse as volumes in your compose file to be able to access them in Disks+. For example:") }}
...
async function handleSubmit() {
  ...
  if (password.value === "passwd") {
    error.value = t("Default password is not allowed to access Disks+. Please change it in Settings.");
    return;
  }
  loading.value = true;
  error.value = "";
...
function mapError(code: string | undefined, remaining: number | undefined, retryAfter: number | undefined): string {
  switch (code) {
    case "default_password":
      return t("Default password is not allowed to access Disks+. Please change it in Settings.");
